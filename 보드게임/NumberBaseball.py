...
# 하단 왼쪽: 숫자 입력란 (st.pills 적용)
with bot_left:
    st.subheader("🎯 숫자 입력")
    
    if not st.session_state.game_over:
        st.info("💡 **버튼을 순서대로 터치**하여 숫자를 조합하세요.")
        
        # 폼(form)을 사용해 터치를 다 한 뒤에 '확인'을 누르도록 함
        with st.form("guess_form", clear_on_submit=True):
            # st.pills를 다중 선택(multi) 모드로 설정
            user_guess_list = st.pills(
                f"{st.session_state.digit_length}개의 숫자를 선택하세요 (중복 불가):",
                options=[str(i) for i in range(10)], # 0부터 9까지의 옵션
                selection_mode="multi"
            )
            
            submitted = st.form_submit_button("확인 (입력 완료)")

        # 확인 버튼을 눌렀을 때의 검증 로직
        if submitted:
            # pills는 리스트 형태(['1', '5', '8'])로 반환하므로 문자열("158")로 합쳐줍니다.
            user_guess = "".join(user_guess_list) if user_guess_list else ""
            
            if len(user_guess) != st.session_state.digit_length:
                st.error(f"⚠️ {st.session_state.digit_length}개의 숫자를 모두 선택해 주세요! (현재 {len(user_guess)}개 선택됨)")
            else:
                # 숫자 중복 검사는 pills의 'multi' 모드 특성상 자동으로 방지됩니다!
